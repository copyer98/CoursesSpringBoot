...
public class KnowledgePointServiceImpl implements KnowledgePointService {
       ...
       /* System.out.println("-----------------------------------------------------------------------");*/
        ...
        switch (contentLength){
            ...
            case 6:
                ...
                String content65=knowledgePoint_content.substring(3,6);
    ...
    */
    @Override
    public Integer addKnowledgePoints(String unit_id,String knowledgePoints) throws Exception{

        String [] kps=knowledgePoints.split(";");
        //获取该单元当前的所有知识点id
        String [] kp_ids=unit_knowledgePointDao.getKnowledgePointsByUnit(unit_id);
        ...
        //当前教学单元的知识点发生了变动，我们需要找出变动并更新
        else{

            //用于判断是否有旧知识点被删除
            int [] aid=new int[kp_ids.length];
            ...
            for (String kp:kps){
                ...
                //用来判断该知识点之前是否存在过，0：未存在；1：存在过
                int count=0;
                int position=0;
                for (String kp_id:kp_ids){
                    KnowledgePoint knowledgePoint=knowledgePointDao.queryKnowledgePointById(kp_id);
                    //如果该知识点之前已存在
                    if (kp.equals(knowledgePoint.getContent())){
                        //不做处理，直接跳过
                        count=1;
                        aid[position]=0;
                        break;
            ...
            for (int i=0;i<aid.length;i++){
                ...
                if (aid[i]==0){
                    ...
                    //位置i对应的知识点已被删除
                    //下面是删除操作
                    int deleteSuccess=knowledgePointDao.putToDustbin(kp_ids[i]);
                    if (deleteSuccess!=1){
                        throw new RuntimeException("删除旧知识点出现异常");
                    }
                    int deleteRelationSuccess=unit_knowledgePointDao.deleteRelation(unit_id,kp_ids[i]);
                    if (deleteRelationSuccess!=1){
                        throw new RuntimeException("删除教学单元、知识点关系关系出现异常");
                    }
                }
            }
    ...
    /**
         *
         * @Date 2018/3/10 15:02
         * @author students_ManagementSchool
         * @param unit_id
         * @param kp
         * @return
         * @since JDK 1.8
         * @condition  根据知识点内容和教学单元id添加知识点及其关联
    */
    @Transactional
    public void addKPAndRelation(String unit_id,String kp){
        KnowledgePoint knowledgePoint=new KnowledgePoint();
        knowledgePoint.setContent(kp);
        String knowledgePoint_id=UUID.randomUUID().toString().replaceAll("-","");
        knowledgePoint.setId(knowledgePoint_id);
        int addSuccess=knowledgePointDao.addKnowledgePoint(knowledgePoint);
        if (addSuccess!=1){
            throw new RuntimeException("添加知识点过程出现错误");
        }
        int addRelationSuccess=unit_knowledgePointDao.addRelation(unit_id,knowledgePoint_id);
        if (addRelationSuccess!=1){
            throw new RuntimeException("添加教学单元，知识点关联过程出现异常");
        }
    }
